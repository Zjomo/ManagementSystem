...
def test_handle_llm_client_exceptions(monkeypatch):
    ...
    rejected = handle_llm_client_exceptions(
        LLMAIBaseError(status_code=403, message="Your request was blocked.")
    )
    assert rejected.status_code == 403
    assert "rejected the actual generation request" in rejected.detail
