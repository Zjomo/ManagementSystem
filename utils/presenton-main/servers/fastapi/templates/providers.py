...
from utils.openai_compatible_client import openai_compatible_default_headers
...
def get_template_provider_spec() -> TemplateProviderSpec:
    ...
    if provider in (
        ...
        if not model:
            raise HTTPException(
                status_code=400,
                detail="Template generation requires a selected model.",
            )
        return TemplateProviderSpec(provider=provider, model=model)

    raise HTTPException(
        status_code=400,
        detail="Template generation only supports OpenAI, Codex, Google, Anthropic, or Custom OpenAI-compatible endpoints.",
    )
...
def _custom_llm_vision_enabled() -> bool:
    return (os.getenv("CUSTOM_LLM_ENABLE_VISION") or "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _custom_llm_prefer_responses() -> bool:
    return (os.getenv("CUSTOM_LLM_PREFER_RESPONSES") or "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _should_try_alternate_openai_like_endpoint(exc: Exception) -> bool:
...
def _should_retry_custom_without_image(exc: Exception) -> bool:
    message = _exception_message(exc).lower()
    return any(
        token in message
        for token in (
...
def _get_custom_openai_like_client() -> AsyncOpenAI:
...
def _read_chat_response_text(response: Any) -> str:
...
async def _call_openai_chat_completions(
    *,
    ...
    output_text = _read_chat_response_text(response)
    if not output_text:
        raise HTTPException(status_code=500, detail="No output from template provider")
    return output_text


async def _call_custom_openai_like(
    *,
    model: str,
    system_prompt: str,
    user_text: str,
    ...
    client = _get_custom_openai_like_client()
    image_for_request = image_bytes if _custom_llm_vision_enabled() else None

    async def call_responses(candidate_image_bytes: Optional[bytes]) -> str:
    ...
    async def call_chat(candidate_image_bytes: Optional[bytes]) -> str:
    ...
    async def call_with_image(candidate_image_bytes: Optional[bytes]) -> str:
        prefer_responses = _custom_llm_prefer_responses()
        primary = call_responses if prefer_responses else call_chat
        fallback = call_chat if prefer_responses else call_responses
        try:
            return await primary(candidate_image_bytes)
        except Exception as exc:
            if not _should_try_alternate_openai_like_endpoint(exc):
                raise
            return await fallback(candidate_image_bytes)

    try:
        return await call_with_image(image_for_request)
    except Exception as exc:
        if image_for_request is not None and _should_retry_custom_without_image(exc):
            return await call_with_image(None)
        raise
    ...
    *,
    ...
    if spec.provider == LLMProvider.CUSTOM:
        return PlainLLMProvider(
            ...
            call=lambda: _call_custom_openai_like(
                model=spec.model,
                system_prompt=system_prompt,
                user_text=user_text,
                image_bytes=image_bytes,
