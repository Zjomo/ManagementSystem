...
SPACY_INSTALL_HINT = (
    "Install it from the FastAPI project with "
    "`cd servers/fastapi && uv run python -m spacy download {model}` "
    "or set MEM0_REQUIRE_SPACY_MODEL=false."
)
...
def _is_qdrant_local_lock_error(exc: BaseException) -> bool:
    err = str(exc)
    return (
        "AlreadyLocked" in type(exc).__name__
        or "already accessed by another instance of Qdrant client" in err
        or "Storage folder" in err
        and "Qdrant client" in err
    )
...
def _spacy_model_available() -> bool:
    ...
    try:
        ...
        LOGGER.warning(
            "Mem0 disabled: spaCy model '%s' is unavailable. %s",
            model,
            SPACY_INSTALL_HINT.format(model=model),
        )
        return False
...
def get_shared_mem0_client() -> Any | None:
    ...
    with _memory_init_lock:
        ...
        try:
            ...
            telemetry_base = os.path.join(mem0_dir, "telemetry", "oss").replace(
                "\\", "/"
            )
            ...
            err = str(exc)
            if _is_qdrant_local_lock_error(exc):
                LOGGER.warning(
                    "Mem0 OSS disabled: local Qdrant storage is already locked by another process. "
                    "Close the other Presenton/FastAPI instance, set a separate MEM0_QDRANT_PATH, "
                    "or keep MEM0_ENABLED=false for local Electron development."
                )
            elif "NoSuchFile" in type(exc).__name__ or "NO_SUCHFILE" in err:
                LOGGER.warning(
