...
export const FileUploadSection: React.FC<FileUploadSectionProps> = ({
  ...
  useEffect(() => {
    const supportedProviders = new Set(["openai", "google", "anthropic", "codex", "custom"]);
    setIsAllowed(supportedProviders.has(llm_config?.LLM || "openai"));
  }, [llm_config]);
    ...
    <div className="md:h-[calc(100vh-310px)] h-[calc(100vh-450px)] relative overflow-hidden">
      ...
      <div className=" max-w-[650px] w-full mx-auto px-2 md:px-0 ">
        ...
        >
          ...
          <button
            type="button"
            className={`flex justify-center gap-1 py-2.5 pl-2 pr-3 bg-white rounded-[80px] ${isAllowed ? "cursor-pointer" : "cursor-not-allowed opacity-60"}`}
        ...
        <div className="mt-4 px-4 py-3 rounded-lg border border-[#EBE9FE]  flex items-start gap-2 shadow-md">
          ...
          <p className="text-sm md:text-base font-medium text-[#20165C] tracking-[-0.13px]">
            <span className="font-bold text-[#5B49A1]">Note:</span> Template generation relies on <span className="font-semibold">vision-capable models</span> and is currently supported by providers: <span className="font-medium text-[#5246C3]">Google</span>, <span className="font-medium text-[#5246C3]">OpenAI</span>, <span className="font-medium text-[#5246C3]">Anthropic</span>, <span className="font-medium text-[#5246C3]">Codex</span>, and OpenAI-compatible <span className="font-medium text-[#5246C3]">Custom</span> endpoints.
            For optimal results, use state-of-the-art vision models from these providers, as performance may degrade with smaller models.
          </p>
        </div>
