...
const PresentonMode = ({ currentStep, setStep }: { currentStep: number, setStep: (step: number) => void }) => {
    ...
    const shouldUseManualModelInput =
        isManualModelProvider || (llmConfig.LLM === 'custom' && modelsChecked && availableModels.length === 0);
            ...
            if (llmConfig.LLM === 'google') {
                ...
                response = await fetch(getApiUrl('/api/v1/ppt/openai/models/available'), {
                    ...
                    body: JSON.stringify({
                        ...
                        api_key: currentApiKey,
                        allow_empty_on_error: llmConfig.LLM === 'custom'
                    }),
                });
            }
        ...
        <div className='w-full max-w-[660px] font-syne pb-10'>
            ...
            <div className='p-3 border border-[#EDEEEF] rounded-[11px] bg-white '>
                ...
                <div className='flex items-start gap-4 mt-4'>
                    ...
                    {!shouldUseManualModelInput && llmConfig.LLM !== 'chatgpt' && llmConfig.LLM !== 'codex' && modelsChecked && availableModels.length > 0 && (
                        <div className="w-full">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
