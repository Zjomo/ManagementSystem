...
// Keep Chromium session/cache files out of the persistent userData directory on
// Windows. Locked or corrupt Cache/QuotaManager files there can make dev startup
// noisy and sometimes prevent the renderer from opening.
if (process.platform === "win32") {
  const runtimeRoot = path.join(
    app.getPath("temp"),
    "presenton-electron-runtime",
    String(process.pid),
  );
  const sessionDataDir = path.join(runtimeRoot, "session-data");
  const cacheDir = path.join(runtimeRoot, "cache");
  const mediaCacheDir = path.join(runtimeRoot, "media-cache");
  ...
  try {
    ...
    console.warn("[Presenton] Failed to configure temporary Electron cache paths", error);
  }
...
function registerDevToolsShortcuts(): void {
  win?.webContents.on("before-input-event", (event, input) => {
    ...
    if (!isDevToolsShortcut || !win || win.isDestroyed()) {
      return;
    }
...
async function startServers(fastApiPort: number, nextjsPort: number): Promise<boolean> {
  try {
    ...
    const fastApi = await startFastApiServer(
      ...
      fastApiPort,
      {
        ...
        MEM0_REQUIRE_SPACY_MODEL: process.env.MEM0_REQUIRE_SPACY_MODEL ?? "false",
...
async function stopServers() {
  if (fastApiProcess?.pid) {
    const fastApiExited =
      fastApiProcess.killed ||
      fastApiProcess.exitCode !== null ||
      (fastApiProcess as any).__presentonExited;
    if (!fastApiExited) {
  ...
  if (nextjsProcess) {
    const nextjsExited =
      nextjsProcess.killed ||
      nextjsProcess.exitCode !== null ||
      nextjsProcess.__presentonExited;
    if ("pid" in nextjsProcess && nextjsProcess.pid && !nextjsExited) {
    ...
    } else if (!nextjsExited && typeof nextjsProcess.close === "function") {
...
app.whenReady().then(async () => {
  ...
  const serversStarted = await startServers(fastApiPort, nextjsPort);
  if (!serversStarted) {
    return;
  }
